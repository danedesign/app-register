...
然后把绿色软件的 `.exe`、`.bat` 或 `.cmd` 文件拖到窗口里。工具会自动：
...
如果拖入多个支持的文件，会一次性全部登记。

`.bat` / `.cmd` 会创建开始菜单快捷方式和“已安装的应用”条目，但不会写入 `App Paths`。`App Paths` 更适合 `.exe`，对批处理文件没有同等的系统行为。
